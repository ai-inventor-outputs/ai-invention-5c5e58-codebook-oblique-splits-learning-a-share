#!/usr/bin/env python3
"""Codebook-FIGS Iter3: Definitive Full K-Sweep Benchmark.

Implements an improved Codebook-FIGS incorporating all iter1-2 lessons:
  - Task-adaptive init (LDA for classification, PCA for regression) + random
  - WLS refinement with orthogonality regularization
  - True FIGS-style simultaneous tree growth (priority queue)
  - 7 alternation rounds with convergence tracking
  - Full K∈{3,5,8,12,20} sweep across all 10 datasets
  - Codebook stability via Hungarian-aligned cosine similarity
...
MAX_RULES = 12
MAX_DEPTH = 4
N_ALTERNATION_ROUNDS = 7
MIN_SAMPLES_LEAF = 5
LAMBDA_VALUES = [0.0, 0.01, 0.1]
TIMEOUT_TOTAL_SEC = 3000  # 50 min hard wall
...
def find_best_codebook_split(
    ...
    Returns: (best_k, best_threshold, best_reduction, idxs_left, idxs_right)
    """
...
# ═══════════════════════════════════════════════════════════════════════════════
# PHASE 5: EFFECTIVE RANK & STABILITY METRICS
# ═══════════════════════════════════════════════════════════════════════════════
...
class CodebookFIGS:
    ...
    # ── FIT: Alternating optimization with convergence tracking ────────────
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, task_type: str = "classification") -> "CodebookFIGS":
        ...
        for round_idx in range(self.n_alternation_rounds):
            # STEP A: Grow trees using FIGS-style simultaneous growth
    ...
    # ── WLS CODEBOOK REFINEMENT ───────────────────────────────────────────

    def _refine_codebook_wls(self, X: np.ndarray, y: np.ndarray) -> None:
        """WLS refinement: X^T y_residual direction."""
    ...
    def _apply_ortho_regularization(self) -> None:
        """Apply soft orthogonality penalty to codebook."""
        if self.ortho_lambda <= 0:
            return
        C = self.codebook_
        K = C.shape[0]
        G = C @ C.T  # Gram matrix K x K
        I_K = np.eye(K)
        # Gradient of ||G - I||_F^2 w.r.t. C is 4*(G - I) @ C
        grad = 4.0 * (G - I_K) @ C
...
def run_xgboost_baseline(
    ...
    """XGBoost baseline (max_depth=4, n_estimators=100)."""
...
def run_lgbm_baseline(
    ...
    """LightGBM baseline (max_depth=4, n_estimators=100)."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# PHASE 10: OUTPUT FORMATTING (exp_gen_sol_out schema)
# ═══════════════════════════════════════════════════════════════════════════════
...
def format_output_for_schema(
    ...
    Schema: {datasets: [{dataset: str, examples: [{input: str, output: str, predict_*, metadata_*}]}]}
    """
    datasets_out = []
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("Codebook-FIGS Iter3: Definitive Full K-Sweep Benchmark")
    logger.info("=" * 70)
    ...
    mini_datasets = load_datasets(str(MINI_DATA_PATH))
    ...
    # ══════════════════════════════════════════════════════════════════════
    # 9a. SMOKE TEST on mini data
    # ══════════════════════════════════════════════════════════════════════
    logger.info("\n=== SMOKE TEST ===")
    # Use _mini datasets from full_data_out.json (200 samples) for smoke test
    # mini_data_out.json only has 3 examples which is too small
    smoke_ds = None
    for name in ["heart_disease_mini", "diabetes_pima_mini"]:
        if name in full_datasets:
            ...
            break
    if smoke_ds is None:
        # Fallback: try any _mini dataset from full_datasets
    ...
    if smoke_ds is not None and smoke_ds["n_samples"] >= 10:
        ...
        folds_smoke = smoke_ds["folds"]
        tt_smoke = smoke_ds["task_type"]

        fold0_mask = folds_smoke == folds_smoke[0]
        # Use a split even if only 1 fold exists in mini data
        ...
        for init_str in ["adaptive", "random"]:
            try:
                model = CodebookFIGS(
                    ...
                    n_alternation_rounds=2, init_strategy=init_str,
                    ortho_lambda=0.01, random_state=42,
                )
                ...
                assert model.codebook_.shape[0] <= 5, f"Bad codebook shape with {init_str}"

                if tt_smoke == "classification":
                    from sklearn.metrics import accuracy_score
                    acc = accuracy_score(y_te, preds)
                    logger.info(f"  Smoke {init_str}: acc={acc:.3f}, complexity={model.complexity_}, "
                                f"trees={len(model.trees_)}")
                else:
                    logger.info(f"  Smoke {init_str}: complexity={model.complexity_}, "
                                f"trees={len(model.trees_)}")
            except Exception:
    ...
    # ══════════════════════════════════════════════════════════════════════
    # 9b. LAMBDA MINI-SWEEP
    # ══════════════════════════════════════════════════════════════════════
    logger.info("\n=== LAMBDA MINI-SWEEP ===")
    lambda_test_ds_names = []
    for candidate in ["heart_disease", "diabetes_regression"]:
        if candidate in full_only:
            lambda_test_ds_names.append(candidate)
    if not lambda_test_ds_names:
        lambda_test_ds_names = list(full_only.keys())[:2]

    lambda_scores: Dict[float, List[float]] = {lam: [] for lam in LAMBDA_VALUES}
    best_lambda = 0.01  # default

    for ds_name in lambda_test_ds_names:
        ds = full_only[ds_name]
        X, y, folds, task_type = ds["X"], ds["y"], ds["folds"], ds["task_type"]
        fold0_val = sorted(np.unique(folds))[0]
        fold0_mask = folds == fold0_val
        ...
        for lam in LAMBDA_VALUES:
            try:
                model = CodebookFIGS(
                    K=8, max_rules=MAX_RULES, max_depth=MAX_DEPTH,
                    n_alternation_rounds=5, init_strategy="adaptive",
                    ortho_lambda=lam, random_state=42,
                )
                ...
                if task_type == "classification":
                    preds = model.predict(X_te)
                    from sklearn.metrics import accuracy_score
                    score = float(accuracy_score(y_te, preds))
                else:
                    preds = model.predict(X_te)
                ...
                logger.info(f"  λ={lam} on {ds_name}: score={score:.4f}")
            except Exception:
                logger.exception(f"Lambda sweep failed for λ={lam} on {ds_name}")

    # Pick best lambda
    valid_lambdas = {lam: scores for lam, scores in lambda_scores.items() if scores}
    if valid_lambdas:
        best_lambda = max(valid_lambdas, key=lambda l: np.mean(valid_lambdas[l]))
    logger.info(f"Best lambda: {best_lambda} (scores: {lambda_scores})")
    ...
    CB_CONFIGS = [
        {"name": "cb_figs_adaptive", "init": "adaptive", "lambda": best_lambda},
        {"name": "cb_figs_random", "init": "random", "lambda": best_lambda},
    ]

    for ds_name, ds_info in sorted(full_only.items()):
        ...
        for fold_idx in range(actual_n_folds):
            ...
            # ── Codebook-FIGS for each config × K ─────────────────────
            for cfg in CB_CONFIGS:
                for K in K_VALUES:
                    ...
                    try:
                        ...
                        all_results.append({
                            "dataset": ds_name, "method": "codebook_figs",
            ...
            for K in K_VALUES:
                ...
                for r in all_results:
                    if (r["dataset"] == ds_name
                            ...
                            and r.get("K") == K
                            and r["method"] == "codebook_figs"):
                        r["codebook_stability_mean"] = mean_sim

    # ══════════════════════════════════════════════════════════════════════
    # 9e. AGGREGATE AND SUMMARIZE
    # ══════════════════════════════════════════════════════════════════════
    logger.info("\n=== AGGREGATING ===")
    ...
    # Schema-compliant method_out.json
    ...
    # Also write detailed_results.json
    detailed = _sanitize_for_json({
        "experiment": "codebook_figs_iter3_full_sweep",
        "results": all_results,
        "summary": summary,
        "config": config_meta,
        "lambda_sweep": lambda_scores,
